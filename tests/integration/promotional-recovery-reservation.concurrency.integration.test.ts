...
describeWithDatabase("Promotional recovery reservation PostgreSQL concurrency", () => {
  it("allows exactly one worker to reserve the final selected promotional credit", async () => {
    ...
    try {
      ...
      const [first, second] = await Promise.all([
        new PromotionalRecoveryReservationService(firstClient).reserve({ shopId, planId, sourceKey: `${shopId}:recovery:1`, now: new Date("2026-06-01T00:00:00.000Z") }),
        new PromotionalRecoveryReservationService(secondClient).reserve({ shopId, planId, sourceKey: `${shopId}:recovery:2`, now: new Date("2026-06-01T00:00:00.000Z") }),
      ]);
      ...
      const persistedReservations = await database.usageReservation.findMany({ where: { shopId } });
      expect(persistedReservations).toHaveLength(1);
      expect(persistedReservations[0]).toMatchObject({ promotionalCreditGrantId: grant.id, quantity: 1, status: "RESERVED" });
      expect((persistedGrant?.quantity ?? 0) - (persistedGrant?.committedQuantity ?? 0) - (persistedGrant?.reservedQuantity ?? 0)).toBe(0);
  ...
  it("deduplicates same-source concurrent reservations across independent clients", async () => {
    const shopId = randomUUID();
    const database = new PrismaClient();
    const firstClient = new PrismaClient();
    const secondClient = new PrismaClient();
    ...
    try {
      const plan = await database.billingPlan.create({
        data: {
          shopifyPlanHandle: `same-source-${shopId}`,
          name: "Same-source concurrency plan",
          kind: "FREE",
          defaultOutboundSoftLimit: 10,
          defaultOutboundHardLimit: 20,
          terminalMessageReservedSlots: 1,
      ...
      campaignId = campaign.id;
      const grant = await database.promotionalCreditGrant.create({
        data: { shopId, campaignId, quantity: 2 },
      });
      await database.merchantPromotionSelection.create({
        data: { shopId, promotionalCreditGrantId: grant.id },
      });

      const sourceKey = `${shopId}:recovery:same-source`;
      const [first, second] = await Promise.all([
        new PromotionalRecoveryReservationService(firstClient).reserve({ shopId, planId, sourceKey, now: new Date("2026-06-01T00:00:00.000Z") }),
        new PromotionalRecoveryReservationService(secondClient).reserve({ shopId, planId, sourceKey, now: new Date("2026-06-01T00:00:00.000Z") }),
      ]);

      expect([first, second].filter((outcome) => outcome.kind === "reserved")).toHaveLength(1);
      expect([first, second].some((outcome) => outcome.kind === "already-reserved")).toBe(true);
      const persistedGrant = await database.promotionalCreditGrant.findUnique({ where: { id: grant.id } });
      const persistedReservations = await database.usageReservation.findMany({ where: { shopId } });
      expect(persistedReservations).toHaveLength(1);
      expect(persistedReservations[0]).toMatchObject({ sourceKey, promotionalCreditGrantId: grant.id, quantity: 1 });
      expect(persistedGrant).toMatchObject({ reservedQuantity: 1, committedQuantity: 0 });
    } finally {
      await database.merchantPromotionSelection.deleteMany({ where: { shopId } }).catch(() => undefined);
      await database.promotionalCreditGrant.deleteMany({ where: { shopId } }).catch(() => undefined);
      if (campaignId) await database.promotionCampaign.delete({ where: { id: campaignId } }).catch(() => undefined);
